...
public class TaskIO {
    ...
    public static void write(TaskList tasks, Writer out) throws IOException {
        try (BufferedWriter bufferedWriter = new BufferedWriter(out)) {
            Task lastTask = tasks.size() > 0 ? tasks.getTask(tasks.size() - 1) : null;
            for (Task t : tasks) {
    ...
    private static Task getTaskFromString(String line) {
        if (line == null || line.isEmpty()) {
            log.error("Attempted to parse an empty or null task string.");
            throw new IllegalArgumentException("Task string cannot be empty or null.");
        }
        ...
        if (title.isEmpty()) {
            log.error("Task title is missing or malformed.");
        ...
        try {
            ...
            log.error("Error parsing task details: " + e.getMessage());
